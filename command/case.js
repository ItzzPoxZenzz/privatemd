...
switch (command) {
case 'menu': case 'help': case 'multi':
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
				...
				break
case 'infobot': case 'sewa': case 'sewabot':
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			reply('Sewa bot??\nWa : wa.me/6285387059588')
break
case 'owner':{
	if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
		...
		for (let i of ownerContact.map(v => v + '@s.whatsapp.net')) {
			...
			ini_list.push({
				...
				hehe = await haruka.sendMessage(from, {
					...
					haruka.sendMessage(from, `Sok Asikk🥱\nCanda Kak >.< Nih.`, text, {quoted: hehe})
				}
			break
...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`Multi bot :~ ${err}`)
if (stdout) {
reply(stdout)
}
})
